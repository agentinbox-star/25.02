...
Integrate Agent Inbox with your Replit apps and agents slug: integrations/replit description: Agent Inbox's Replit integration
...
Agent Inbox is a "connector" integration on Replit, meaning your Replit apps and agents can use it directly.

### Setup

Navigate to the Integrations tab in the Replit builder. Find Agent Inbox and click Connect/Sign in. Then paste in your Agent Inbox API key from the Agent Inbox Console. That's it.

## How it works

Replit uses Mastra, a TypeScript framework for building agents. Every time you create a new app/agent Replit will generate a set of Mastra tools to interface with the Agent Inbox API, based on your use case and requirements.

Below is a collection of prebuilt tools that Replit can use out of the box to serve virtually any use case.
...
Agent Inbox is the email inbox platform for AI agents. It gives agents their own email inboxes, like Gmail does for humans. Unlike other email APIs, Agent Inbox is for both sending and receiving, supporting two-way email conversations.

Email is critical to identity and communication on the internet. Much of the context that people use for their work exists within email. Agent Inbox enables AI agents to meet people where they work and use the internet like humans do.

### Features

1. **Programmatic inboxes:** Create, manage, and delete inboxes via API
2. **Threads and replies:** Facilitate 2-way email conversations with persistent and relevant context
...
6. **Language/Framework SDKs:** Use your preferred language and framework
...
4. **Identify and authenticate:** Agents can sign up, receive 2FA codes, and authenticate with any application
...
These are just a few select verticals, but we have seen Agent Inbox be effective in automating any email task across every function. If a human does it with email, it can be automated with Agent Inbox.